...
const authSlice = createSlice({
    ...
    extraReducers: {
        ...
        [thunkSignIn.fulfilled]: (state, action) => {
            ...
            if (accessToken) {
                localStorage.setItem(PreferenceKeys.adminSHOME_accessToken, accessToken);
            ...
            if (accountData) {
                ...
                if (accessToken) {
                    localStorage.setItem(
                        PreferenceKeys.adminSHOME_accessToken,
                        accessToken
                    );
                    updateAxiosAccessToken(accessToken);
                }
        ...
        [thunkSignOut.fulfilled]: (state, action) => {
            const { result } = action.payload;
            if (result === "success") {
                localStorage.removeItem(PreferenceKeys.adminSHOME_accessToken);
                state.currentAccount = {};
