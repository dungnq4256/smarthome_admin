import { thunkGetHomesList, thunkUpdateHomeData } from "app/homeSlice";
...
ModalEditHome.propTypes = {
    show: PropTypes.bool,
    onClose: PropTypes.func,
    homeItem: PropTypes.object,
};

ModalEditHome.defaultProps = {
    show: false,
    onClose: null,
    homeItem: {},
};
...
function ModalEditHome(props) {
    ...
    // MARK: --- Params ---
    const { show, onClose, homeItem } = props;
    const [showing, setShowing] = useState(true);

    const { isUpdatingHomeData } = useSelector((state) => state?.home);
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            const params = { _id: homeItem._id, ...values };
            try {
    ...
    useEffect(() => {
        if (homeItem) {
            formik.getFieldHelpers("name").setValue(homeItem.name);
            formik.getFieldHelpers("address").setValue(homeItem.address);
        }
    }, [homeItem]);
